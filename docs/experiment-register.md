...
- X2 objective matrix: CE-only (lambda 0), AURC-weighted CE surrogate only (lambda 1), and blend (lambda 0.5), all initialized from the same pinned pretrained ModernBERT revision with fresh optimizers. Follow-up repetitions only after a promising, interpretable signal.
...
- Training infrastructure: use the configured `ssh uab-gpu` cluster for research runs, recording scheduler allocation, GPU availability, durable storage, interruption/resume, wall time and total cost. See [training infrastructure plan](training-infrastructure-plan.md). Modal runs remain historical preliminary evidence.
