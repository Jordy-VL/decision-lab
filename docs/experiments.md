...
For every arm and selected checkpoint, save raw logits, labels, IDs, group IDs, source/type and complete probabilities for the development (validation) and calibration partitions. Keep these as immutable, paired artifacts alongside the checkpoint and resolved config. Select the best checkpoint by development AUGRC because selective confidence ordering is the primary research target and AUGRC avoids known AURC ranking pathologies. Report development AURC, NLL and Brier as secondary diagnostics; do not use NLL as the primary selection rule because that would favor CE's directly optimized likelihood objective. Accuracy must not determine checkpoint selection because it ignores probability quality and confidence ordering. The current Trainer configuration implements development-AUGRC selection and retains the final-budget checkpoint separately. Never use final-test labels for checkpoint selection.
