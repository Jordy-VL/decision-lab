...
6. **Later hypotheses:** vision with a small SigLIP encoder/fusion, multi-turn rollouts, RL abstention under domain shift, and numeric prediction only if a use case warrants a new primitive. [ModernVBERT](https://huggingface.co/ModernVBERT/modernvbert) is a candidate multimodal backbone/reference for the document and image decision experiments.
...
ModernVBERT is recorded as a deferred multimodal reference only. It does not
change the current text-only X2 arms, their data, or their evaluation
protocol. Before using it, pin the model revision, inspect its image/text
input contract and license, measure context and GPU requirements, and define
a matched OCR-only versus image-plus-text comparison.
